...
export const runDailyPayouts = async () => {
    ...
    return { success: true, processedCount, totalPayout };
};
...
export const processBatchPayouts = async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: `Payout job completed. Processed ${result.processedCount} wallets. Total: KES ${result.totalPayout}`,
            data: result
        });
    } catch (error) {
